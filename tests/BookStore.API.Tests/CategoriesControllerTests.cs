...
namespace BookStore.API.Tests
{
    public class CategoriesControllerTests
    {
        ...
        public async void GetAll_ShouldReturnOk_WhenExistCategory()
        {
            ...
            _mapperMock.Setup(m => m.Map<IEnumerable<CategoryResult>>(
                It.IsAny<List<Category>>())).Returns((IEnumerable<CategoryResult>)dtoExpected);
        ...
        public async void GetAll_ShouldReturnOk_WhenDoesNotExistAnyCategory()
        {
            ...
            _categoryServiceMock.Setup(c => c.GetAll()).ReturnsAsync(categories);
            _mapperMock.Setup(m => m.Map<IEnumerable<CategoryResult>>(It.IsAny<List<Category>>())).Returns((IEnumerable<CategoryResult>)dtoExpected);
        ...
        public async void GetById_ShouldReturnOk_WhenCategoryExist()
        {
            ...
            _mapperMock.Setup(m => m.Map<CategoryResult>(It.IsAny<Category>())).Returns(dtoExpected);
        ...
        {
            var category = CreateCategory();
            var categoryAddDto = new CategoryAdd() { Name = category.Name };
            var categoryResultDto = MapModelToCategoryResultDto(category);

            _mapperMock.Setup(m => m.Map<Category>(It.IsAny<CategoryAdd>())).Returns(category);
            _categoryServiceMock.Setup(c => c.Add(category)).ReturnsAsync(category);
            _mapperMock.Setup(m => m.Map<CategoryResult>(It.IsAny<Category>())).Returns(categoryResultDto);
        ...
        {
            var categoryAddDto = new CategoryAdd();
        ...
        {
            var category = CreateCategory();
            var categoryEditDto = new CategoryEdit() { Id = category.Id,  Name = "Test" };

            _mapperMock.Setup(m => m.Map<Category>(It.IsAny<CategoryEdit>())).Returns(category);
        ...
        {
            var categoryEditDto = new CategoryEdit() { Id = 1,  Name = "Test" };
        ...
        {
            var categoryEditDto = new CategoryEdit() { Id = 1 };
            _categoriesController.ModelState.AddModelError("Name", "The field name is required");
        ...
        private CategoryResult MapModelToCategoryResultDto(Category category)
        {
            var categoryDto = new CategoryResult()
            {
                Id = category.Id,
                Name = category.Name
            };
        ...
        private List<CategoryResult> MapModelToCategoryListDto(List<Category> categories)
        {
            var listCategories = new List<CategoryResult>();
