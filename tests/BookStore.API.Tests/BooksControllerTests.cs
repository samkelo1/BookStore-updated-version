...
namespace BookStore.API.Tests
{
    public class BooksControllerTests
    {
        ...
        public async void GetAll_ShouldReturnOk_WhenExistBooks()
        {
            ...
            _mapperMock.Setup(m => m.Map<IEnumerable<BookResult>>(It.IsAny<List<Book>>())).Returns((IEnumerable<BookResult>)dtoExpected);
        ...
        public async void GetById_ShouldReturnOk_WhenBookExist()
        {
            ...
            _mapperMock.Setup(m => m.Map<BookResult>(It.IsAny<Book>())).Returns(dtoExpected);
        ...
        public async void GetBooksByCategory_ShouldReturnOk_WhenBookWithSearchedCategoryExist()
        {
            ...
            _mapperMock.Setup(m => m.Map<IEnumerable<BookResult>>(It.IsAny<IEnumerable<Book>>())).Returns((IEnumerable<BookResult>)dtoExpected);
        ...
        {
            var book = CreateBook();
            var bookAddDto = new BookAdd() { Name = book.Name };
            var bookResultDto = MapModelToBookResultDto(book);

            _mapperMock.Setup(m => m.Map<Book>(It.IsAny<BookAdd>())).Returns(book);
            _bookServiceMock.Setup(c => c.Add(book)).ReturnsAsync(book);
            _mapperMock.Setup(m => m.Map<BookResult>(It.IsAny<Book>())).Returns(bookResultDto);
        ...
        {
            var book = CreateBook();
            var bookEditDto = new BookEdit() { Id = book.Id, Name = "Test" };

            _mapperMock.Setup(m => m.Map<Book>(It.IsAny<BookEdit>())).Returns(book);
        ...
        {
            var bookEditDto = new BookEdit() { Id = 1, Name = "Test" };
        ...
        {
            var bookEditDto = new BookEdit() { Id = 1 };
            _booksController.ModelState.AddModelError("Name", "The field name is required");
        ...
        public async void SearchBookWithCategory_ShouldReturnOk_WhenBookWithSearchedValueExist()
        {
            ...
            _mapperMock.Setup(m => m.Map<IEnumerable<BookResult>>(It.IsAny<List<Book>>())).Returns((IEnumerable<BookResult>)bookResultList);
        ...
        private BookResult MapModelToBookResultDto(Book book)
        {
            var bookDto = new BookResult()
            {
                Id = book.Id,
                Name = book.Name,
                Author = book.Author,
        ...
        private List<BookResult> MapModelToBookResultListDto(List<Book> books)
        {
            var listBooks = new List<BookResult>();
